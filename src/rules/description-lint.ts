 ...
 */
const INSTRUCTION_PATTERNS: { pattern: RegExp; why: string }[] = [
  {
    pattern:
  ...
  {
    pattern:
      /\b(send|forward|include|attach|pass|transmit|post)\b[^.]{0,60}\b(api[\s_-]?keys?|tokens?|passwords?|secrets?|credentials?|\.env|ssh[\s_-]?keys?|private[\s_-]?keys?)\b/i,
...
export const descriptionLint: Rule = {
  ...
  description:
    "Tool descriptions and server instructions that direct the model, contain invisible characters, or include credentials.",
